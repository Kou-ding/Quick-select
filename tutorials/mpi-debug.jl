...
while ((winCondition1Buff[1] != 1) && (winCondition2Buff[1]!=winCondition1Buff[1]))
    ...
    if rank in 1:size-1 && subA[rank] != []
        ...
        print("Pivot of process $rank: $(pivot[1])\n")
    ...
    println("winCondition1Buff $rank: $(winCondition1Buff[1])")
    println("winCondition2Buff $rank: $(winCondition2Buff[1])")
    # Wait for all processes to finish
    MPI.Barrier(comm)
end
...
#= The working quick-select-mpi.jl without removed barriers
# Initialize the array A[]
function init_array()
    # Initialize array
    global A=zeros(Int64,0)
